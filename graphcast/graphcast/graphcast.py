...
class GraphCast(predictor_base.Predictor):
  ...
  * Mesh2Grid graph: Graph that contains all nodes. This graph is strictly
  ...
  def __call__(self,
    ...
    context, inputs = inputs.isel(time=[0, 1]), inputs.isel(time=[2, 3])
    context_forcings = inputs.isel(time=0)[forcings.keys()]
    ...
    context_grid_nodes = self._inputs_to_grid_node_features(context, context_forcings)
    ...
    self._run_grid2mesh_gnn(context_grid_nodes)
    embeddings = self._grid2mesh_gnn.embeddings
    ...
    latent = self._mesh2grid_gnn.latent
    ...
    return self._grid_node_outputs_to_prediction(
        output_grid_nodes, targets_template), embeddings, latent
