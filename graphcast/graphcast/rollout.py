...
def chunked_prediction(
  ...
  for prediction_chunk in chunked_prediction_generator(
      ...
      verbose=verbose):
    chunks_list.append(jax.device_get(prediction_chunk[0]))
    embeddings.append(prediction_chunk[1])
    latent.append(prediction_chunk[2])
  return xarray.concat(chunks_list, dim="time"), np.concatenate(embeddings), np.concatenate(latent)
...
def chunked_prediction_generator(
  ...
  for chunk_index in range(num_chunks):
    ...
    predictions, embeddings, latent = predictor_fn(
        rng=this_rng,
    ...
    if output_datetime is not None:
      predictions.coords["datetime"] = output_datetime.isel(
          time=target_slice)
    yield predictions, embeddings, latent
    del predictions, embeddings, latent
